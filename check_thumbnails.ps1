    ...
    if ($totalProjectIssues -eq 0) {
        ...
        if ($projectIssues.MissingRegular.Count -gt 0) {
            ...
            $projectIssues.MissingRegular | ForEach-Object {
                ...
                $thumbPath = Join-Path $regularThumbsDir $thumbName
                $fixCommands += "ffmpeg -y -i ""$videoPath"" -ss 00:00:02.000 -update 1 -frames:v 1 -vf ""scale=$($thumbWidth):$($thumbHeight):force_original_aspect_ratio=decrease"" ""$thumbPath"""
            }
        }
            ...
            $projectIssues.MissingEdit | ForEach-Object {
                ...
                try {
                    ...
                    for ($i = 1; $i -le 10; $i++) {
                        ...
                        $thumbName = "${baseName}_${i}.jpg"
                        $thumbPath = Join-Path $editThumbsDir $thumbName
                        $fixCommands += "ffmpeg -ss $timestamp -i ""$videoPath"" -update 1 -vframes 1 -vf ""scale=$($thumbWidth):$($thumbHeight):force_original_aspect_ratio=decrease"" -y ""$thumbPath"" >nul 2>&1"
                    }
                } catch {
